...
pub fn run(conn: &Connection, cv_id: i32) -> Result<()> {
    ...
    let mut stmt = conn.prepare(
        "WITH candidates AS (
            SELECT
                j.id, j.title, j.content, j.content_hash,
                array_cosine_similarity(e.embedding, c.embedding) AS similarity,
                ROW_NUMBER() OVER (
                    PARTITION BY j.content_hash
                    ORDER BY array_cosine_similarity(e.embedding, c.embedding) DESC, j.id ASC
                ) AS rn
            FROM jobs j
        ...
        SELECT id, title, content, content_hash, similarity
        FROM candidates
        WHERE rn = 1
    ...
    println!("Ranking {total} distinct candidate jobs for cv_id = {cv_id}...");

    for (i, (job_id, title, raw_content, content_hash, similarity)) in rows.into_iter().enumerate() {
        ...
        match ds.score(&content) {
            Ok((scoring, usage, elapsed)) => {
                ...
                let dup_count = write_success_with_duplicates(conn, cv_id, &content_hash, &scoring, similarity)?;
                write_metrics(conn, cv_id, job_id, &usage, cost, elapsed)?;
                println!("{}", serde_json::to_string_pretty(&scoring)?);
                if dup_count > 1 {
                    println!("  (propagated to {dup_count} duplicate postings)");
                }
            }
            Err(e) => {
...
fn write_success_with_duplicates(
    conn: &Connection,
    cv_id: i32,
    content_hash: &str,
    scoring: &AtsScoring,
    ...
    let mut stmt = conn.prepare(
        "SELECT id FROM jobs WHERE content_hash = ? AND is_active = true",
    )?;
    ...
    for &dup_id in &duplicate_ids {
        conn.execute(
            ...
            params![
                cv_id, dup_id, scoring.ats_score, scoring.recruiter_score, scoring.remote_type,
